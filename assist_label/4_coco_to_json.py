...
class Total_example_json:
    ...
    def add_json_coor(self, object_json_coor, label_class):
...
if __name__ == "__main__":
    ...
    fail = 0 
    json_output_folder = r"D:\label_ship_1102"
    os.makedirs(json_output_folder, exist_ok=True)
    # json_output_folder = "test_output"
    for filename in tqdm(os.listdir(folder_all), total = len(os.listdir(folder_all))):
        count +=1 
        # if count <= 22000:
        #     continue 
        print (count)
        try: 
            filepath = os.path.join(folder_all, filename)
            filename_id = filename.split(".")[0]
            if filename.endswith('.jpg'):
                created = False
                txt_filepath = filepath.replace(".jpg", ".txt")
                ...
                for yolo_coor in yolo_coor_ls:
                    ...
                    if int(one_object.class_id) in mapping.keys():
                        if created == False:
                            ...
                            res.add_image_width_height(imageWidth = width, 
                                imageHeight = height)
                            created = True
                        res.add_json_coor(one_object_json_coor, label_class=mapping[int(one_object.class_id)])
